...
] # cogemos el archivo json y queda como arreglo de diccionarios.

def estadisticasAutor ():
    resultado = {} # Creamos diccionario vacio para almacenar el autor
    for objeto in libros:
        autor = objeto.get("author") # obtener el valor de la clave autor del diccionario
        if autor not in resultado:
            resultado[autor] = 1 #este diccionario almacena la clave autor y el valor 1
        else:
            cantidad_libros = resultado.get(autor)
            resultado[autor] = cantidad_libros+1
    print(resultado)

def estadisticasFecha ():
    total_años = []
    resultado_años = {}
    for objeto in libros:
        año = str(objeto.get("year"))
        total_años.append(año)
        if año not in resultado_años:
          resultado_años[año] = 1
        else:
          total_libros = resultado_años.get(año)
          resultado_años[año] = total_libros+1
    print(resultado_años)
    print(total_años)

def estadisticas_pais ():
    resultado_pais = {}
    for objeto in libros:
        resultado_lenguaje = {}
        pais = objeto.get("country")
        lenguaje = objeto.get("language")
        paginas = objeto.get("pages")
        if pais not in resultado_pais:
          resultado_lenguaje[lenguaje] = paginas
          resultado_pais[pais] = resultado_lenguaje
        else:
          libros_pais = resultado_pais.get(pais)
          libros_pais[lenguaje] = paginas
          resultado_pais[pais] = libros_pais
